...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Examination address (e.g. %1)").arg("PwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwg"));
#endif
...
{
    // return if URI is not valid or is no examination: URI
    if(!uri.isValid() || uri.scheme() != QString("examination"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::EXAM, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert examination:// to examination:
    //
    //    Cannot handle this later, because examination:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("examination://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "examination:");
    }
...
{
    QString ret = QString("examination:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::EXAM, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open examination.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Examination.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Examination.lnk
...
{
    return GetAutostartDir() / "examination.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a examination.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Examination\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the examination app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add examination app to startup item list
