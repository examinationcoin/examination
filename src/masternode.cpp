...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if(Params().NetworkID() == CBaseChainParams::MAIN) {
        if(addr.GetPort() != 36003) return false;
    } else if(addr.GetPort() == 36003) return false;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 5000 EXAM tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
    ...
    {
        CBlockIndex* pMNIndex = (*mi).second; // block for 5000 EXAM tx -> 1 confirmation
