...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop Examination server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Examination server stopping";
}
...
{ //  category              name                      actor (function)         okSafeMode threadSafe reqWallet
  //  --------------------- ------------------------  -----------------------  ---------- ---------- ---------
    ...
    { "examination",               "masternode",             &masternode,             true,      true,       false },
    { "examination",               "masternodelist",         &masternodelist,         true,      true,       false },
    { "examination",               "mnbudget",               &mnbudget,               true,      true,       false },
    { "examination",               "mnbudgetvoteraw",        &mnbudgetvoteraw,        true,      true,       false },
    { "examination",               "mnfinalbudget",          &mnfinalbudget,          true,      true,       false },
    { "examination",               "mnsync",                 &mnsync,                 true,      true,       false },
    { "examination",               "spork",                  &spork,                  true,      true,       false },
#ifdef ENABLE_WALLET
    { "examination",               "darksend",               &darksend,               false,     false,      true  }, /* not threadSafe because of SendMoney */
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use examinationd, or the -server option to examination-qt, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
...
std::string HelpExampleCli(string methodname, string args){
    return "> examination-cli " + methodname + " " + args + "\n";
}

std::string HelpExampleRpc(string methodname, string args){
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
        "\"method\": \"" + methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:36002/\n";
}
