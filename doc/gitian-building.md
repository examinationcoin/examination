...
*Setup instructions for a gitian build of Examination using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Examination
...
- [Building Examination](#building-examination)
...
Clone the git repositories for examination and gitian and then checkout the examination version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/examination/examination.git
cd examination
...
Follow the instructions in [doc/release-process.md](release-process.md) in the examination repository
...
To build Examination (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the examination repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/examination/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/examination/examination
...
For example:
```bash
URL=https://github.com/crowning-/examination.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit examination=${COMMIT} --url examination=${URL} ../examination/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit examination=${COMMIT} --url examination=${URL} ../examination/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit examination=${COMMIT} --url examination=${URL} ../examination/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/examination-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/examination-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/examination-build.assert
```
...
[examination/gitian.sigs](https://github.com/examinationpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
